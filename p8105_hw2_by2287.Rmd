...
`r (nyc_transit %>% select(entrance_type, vending, entry, line, station_name) %>% filter(entry == 1 & vending == "NO") %>% distinct() %>% count() / nyc_transit %>% distinct(line,station_name) %>% count())`
